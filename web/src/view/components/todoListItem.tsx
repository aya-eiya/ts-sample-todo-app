...
export default class TodoListItem extends React.Component<TodoListItemProps, TodoListItemState> {
  ...
  render() {
    ...
    <div className="p-1 h-10 w-72 content-center">{item.schedule.toUTCString()}</div>
  ...
  <li className="flex flex-1 flex-row m-1 items-center" id={`todoItem_${item.id}`}>
    <div className="p-1 h-10 w-36 items-center"><input className="w-full bg-gray-200 rounded focus:outline-none focus:ring focus:border-blue-300 m-1 h-8 p-1 pl-2" type="text" name="title" value={editItem.title} onChange={this.onTitleChange} /></div>
    <div className="p-1 h-10 w-72 items-center"><input className="w-full bg-gray-200 rounded focus:outline-none focus:ring focus:border-blue-300 m-1 h-8 p-1 pl-2" type="text" name="schedule" value={editItem.schedule} onChange={this.onScheduleChange} /></div>
